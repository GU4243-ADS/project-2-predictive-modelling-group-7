...
gbm.pred <- predict(gbm_train, newdata = test_set[-55], n.trees = 1, distribution = "bernoulli", interaction.depth = 3)
mean(gbm.pred)
```
...
library(gbm)
tm_train<- system.time(gbm_train <- gbm.fit(x = training_sift[-100],
                                  y = label_sift, 
                                  n.trees = 1000,
                                  distribution = "bernoulli",
                                  interaction.depth = 3))
...
# Predicting the Test set results
gbm.pred <- predict(gbm_train, newdata = test_sift[-100], n.trees = 1, distribution = "bernoulli", interaction.depth = 3)
mean(gbm.pred)
```
