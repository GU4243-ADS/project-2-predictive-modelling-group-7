...
cv_sift = lapply(folds_sift, function(x) {
  ...
  gbm_sift <- gbm.fit(x = training_fold_sift[-100],
                                  y = training_fold_sift$label, 
                                  n.trees = 1000,
                                  distribution = "bernoulli",
                                  interaction.depth = 3,
                                  bag.fraction = 0.5,
  ...
  pred_sift = predict(gbm_sift, newdata = test_fold_sift[-100], n.trees = 1000)
  pred_sift = (pred_sift >= 0.5)
  cm_sift = table(test_fold_sift[, 55], pred_sift)
  accuracy_sift = (cm_sift[1,1] + cm_sift[2,2]) / (cm_sift[1,1] + cm_sift[2,2] + cm_sift[1,2] + cm[2,1])
